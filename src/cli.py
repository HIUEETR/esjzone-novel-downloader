    ...
    while True:
        ...
        if choice == "修改账号":
            new_username = questionary.text(
                "请输入账号：", default=current_username
            ).ask()
...
def edit_download_menu():
    while True:
        ...
        choice = questionary.select(
            "下载设置", choices=choices, instruction="⌈ 使用↑↓选择 ⌋"
        ).ask()

        if choice.startswith("修改下载目录"):
            val = questionary.text(
                "请输入下载目录：", default=dl_config.get("dir", "downloads")
            ).ask()
            if val:
            ...
            val = questionary.text(
                "请输入最大线程数：", default=str(dl_config.get("max_threads", 5))
            ).ask()
            ...
            val = questionary.text(
                "请输入超时时间（秒）：",
                default=str(dl_config.get("timeout_seconds", 180)),
            ).ask()
            if val and val.isdigit():
            ...
            val = questionary.text(
                "请输入最大重试次数：", default=str(dl_config.get("retry_attempts", 3))
            ).ask()
            if val and val.isdigit():
...
def edit_log_menu():
    while True:
        ...
        choice = questionary.select(
            "日志设置", choices=choices, instruction="⌈ 使用↑↓选择 ⌋"
        ).ask()
        ...
        if choice.startswith("修改日志级别"):
            ...
            val = questionary.text(
                "请输入日志目录：", default=log_config.get("dir", "./logs")
            ).ask()
            if val:
            ...
            val = questionary.text(
                "请输入保留天数：", default=str(log_config.get("retention", 30))
            ).ask()
            if val and val.isdigit():
...
def edit_cookie_menu():
    while True:
        ...
        choice = questionary.select(
            "Cookie 设置", choices=choices, instruction="⌈ 使用↑↓选择 ⌋"
        ).ask()

        if choice.startswith("修改 Cookie 路径"):
            val = questionary.text(
                "请输入 Cookie 文件路径：",
                default=cookie_config.get("path", "data/cookies.json"),
            ).ask()
            if val:
...
def favorite_menu(downloader, favorites_manager):
    ...
    sort_by = questionary.select(
        "请选择收藏夹排序方式：", choices=sort_choices, instruction="⌈ 使用↑↓选择 ⌋"
    ).ask()
    ...
    while True:
        ...
        for idx, novel in enumerate(current_novels):
            ...
            label = (
                f"{abs_idx:>4}. {display_title}   |   {display_latest}   |   {update}"
            )
        ...
        selection = questionary.select(
            "我的收藏夹", choices=choices, instruction="⌈ 使用↑↓翻页/选择，回车确认 ⌋"
        ).ask()
    ...
    while True:
        ...
        if choice == "从网址获取小说":
            url = questionary.text(
                "格式为 https://www.esjzone.one/detail/123456789.html\n请输入小说网址："
            ).ask()
            if url:
                    ...
                    if range_input and range_input.strip():
                        try:
                            if "-" in range_input:
                                ...
                                single_chapter = int(range_input.strip())
                                if single_chapter < 1:
                                    single_chapter = 1
